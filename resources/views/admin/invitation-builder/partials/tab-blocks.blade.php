...
<ul class="list-group mb-3" id="ibBlocksSortable">
	@foreach($activeBlocks as $blockKey)
		...
		<li class="list-group-item ib-block-item d-flex align-items-center gap-2" draggable="true" data-block="{{ $blockKey }}">
...
<div class="row g-2" id="ibBlocksAvailable">
	@foreach($inactiveBlocks as $blockKey)
		@php $block = $catalog['information_blocks'][$blockKey]; @endphp
		<div class="col-md-6">
			<button type="button" class="btn btn-outline-secondary w-100 text-start ib-block-add" data-block="{{ $blockKey }}" data-icon="{{ $block['icon'] }}" data-label="{{ $block['label_ar'] }}" data-desc="{{ $block['description_ar'] }}">
