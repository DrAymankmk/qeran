		...
		<div class="d-flex align-items-center gap-2">
			...
			<div class="flex-grow-1">
				...
				<span class="text-muted"
					style="font-size: 11px;">{{ $block['description_ar'] }}</span>
			</div>
			<input type="hidden" name="blocks[]" value="{{ $blockKey }}" class="ib-preview-field">
			@if(isset($schemas[$blockKey]))
			<button type="button" class="btn btn-sm btn-outline-secondary ib-block-toggle"
				title="{{ __('admin.ib-block-edit') }}">
				<i class="mdi mdi-pencil-outline"></i>
			</button>
			@endif
			<button type="button" class="btn btn-sm btn-outline-danger ib-block-remove"
				title="{{ __('admin.ib-block-remove') }}">×</button>
		</div>
		@if(isset($schemas[$blockKey]))
...
<script type="application/json" id="ibBlockSchemasJson">
@json($schemas)
</script>