@once
	<script>
		window.applyInvitationQrUrl = function (container, qrUrl, filename) {
			...
			var section = container.querySelector('.qr-section, .qr-guest-card') || container;
			var img = section.querySelector('#invitationQrImage, .invitation-guest-qr-image, img');
			...
			if (!img) {
				...
				img.className = 'invitation-guest-qr-image';
		...
		function downloadInvitationQr(button) {
			...
			var section = button.closest('.qr-section, .qr-guest-card');
