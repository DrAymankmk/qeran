...
.wi-env-shape-european .wi-env-top-flap,
.wi-env-shape-european .wi-env-photo-flap {
	height: 58%;
	clip-path: polygon(0 0, 50% 94%, 100% 0);
}
.wi-env-shape-european .wi-env-pocket {
	height: 52%;
	clip-path: polygon(0 0, 50% 32%, 100% 0, 100% 100%, 0 100%);
}
.wi-env-shape-european .wi-env-side { height: 58%; }
...
.wi-env-shape-luxe .wi-env-fold-line {
	opacity: 0.85;
	height: 2px;
	background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--wi-gold) 50%, var(--env-paper-dark)), transparent);
}
