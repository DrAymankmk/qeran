                    ...
                    <div class="table-responsive mt-2">
                        <table class="table table-hover datatable dt-responsive nowrap"
                            ...
                            @foreach($appSettings as $appSetting)
                                ...
                                <tr>
                                    <td><a href="javascript: void(0);" class="text-body fw-bold">{{$appSetting->id}}</a></td>
                                    <td>
                                        <a target="_blank" href="{{$appSetting->img}}">

                                        <img class=" header-profile-user" src="{{$appSetting->image()}}" alt="Header Avatar">
                                        </a>
                                    </td>
...
{{--                                            @can('edit_categories')--}}
                                                ...
                                                <a href="{{route('app-settings.edit',$appSetting->id)}}" title="{{__('admin.edit')}}" class="text-success"><i
                                                        class="mdi mdi-pencil font-size-18"></i></a>
{{--                                            @endcan--}}
{{--                                            @can('delete_categories')--}}

                                                <a onclick="openModalDelete({{$appSetting->id}})" title="{{__('admin.delete')}}" class="text-danger"><i
...
@section('extra-js')
    ...
    <script>
        function openModalDelete(category_id) {
            $('.action_form').attr('action', '{{route('app-settings.destroy', '')}}' + '/' + category_id);
