...
(function() {
	...
	window.renderInvitationHubFilesGrid = function(data) {
		...
		if (!hubFiles || !hubFiles.length) {
			el.innerHTML = '<p class="text-muted small mb-0">{{ __("admin.no-data-available") }}</p>';
			return;
		}
		...
		hubFiles.forEach(function(f) {
			...
			var badgeClass = uploader === 'admin' ? 'bg-primary' : (uploader === 'user' ? 'bg-info' : 'bg-secondary');

			var preview = '';
			var ft = parseInt(f.file_type, 10);
			if (ft === FT_IMAGE || ft === FT_GIF) {
				preview = '<a href="' + safeUrl + '" target="_blank" rel="noopener">' +
					'<img src="' + safeUrl + '" alt="" loading="lazy" />' +
					'</a>';
			} else if (ft === FT_VIDEO) {
				preview = '<video src="' + safeUrl + '" controls playsinline preload="metadata" muted></video>';
			} else if (ft === FT_AUDIO) {
				preview = '<audio src="' + safeUrl + '" controls preload="metadata"></audio>';
			} else {
				preview = '<a href="' + safeUrl + '" target="_blank" rel="noopener" class="btn btn-sm btn-outline-secondary">{{ __("admin.hub-file-open") }}</a>';
			}

			html += '<div class="invitation-hub-file-card">' +
				'<div class="invitation-hub-file-card__preview">' + preview + '</div>' +
				'<div class="invitation-hub-file-card__meta">' +
				'<span class="badge bg-dark">' + typeLabel + '</span><br />' +
				'<span class="badge ' + badgeClass + '">' + upLabel + '</span>' +
				'</div></div>';
		});
		html += '</div>';
		el.innerHTML = html;
