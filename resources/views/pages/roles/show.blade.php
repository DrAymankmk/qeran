@extends('layouts.app')
@section('content')

<!-- start page title -->
<div class="row">
	<div class="col-12">
		<div class="page-title-box d-sm-flex align-items-center justify-content-between">
			<h4 class="mb-sm-0 font-size-18">{{__('admin.role-details')}}</h4>

			<div class="page-title-right">
				<ol class="breadcrumb m-0">
					<li class="breadcrumb-item"><a
							href="{{route('admin.dashboard')}}">{{__('admin.Dashboard')}}</a>
					</li>
					<li class="breadcrumb-item"><a
							href="{{route('roles.index')}}">{{__('admin.roles')}}</a>
					</li>
					<li class="breadcrumb-item active">{{__('admin.role-details')}}</li>
				</ol>
			</div>
		</div>
	</div>
</div>
<!-- end page title -->

<div class="row">
	<div class="col-12">
		<div class="card">
			<div class="card-body">
				<h4 class="mb-3">{{__('admin.role-information')}}</h4>

				<div class="mb-3">
					<label class="form-label fw-bold">{{__('admin.id')}}:</label>
					<p class="mb-0">{{$role->id}}</p>
				</div>

				<div class="mb-3">
					<label class="form-label fw-bold">{{__('admin.name')}}:</label>
					<p class="mb-0">{{$role->name}}</p>
				</div>

				<div class="mb-3">
					<label class="form-label fw-bold">{{__('admin.permissions')}}:</label>
					<div>
						@if($role->permissions->count() > 0)
						@foreach($role->permissions as $permission)
						<span
							class="badge bg-info me-1 mb-1 fw-bold font-size-18">{{$permission->name}}</span>
						@endforeach
						@else
						<span
							class="badge bg-danger">{{__('admin.no-permissions')}}</span>
						@endif
					</div>
				</div>

				<div class="mb-3">
					<label class="form-label fw-bold">{{__('admin.created_at')}}:</label>
					<p class="mb-0">
						{{Carbon\Carbon::parse($role->created_at)->locale(app()->getLocale())->translatedFormat('l dS F G:i - Y')}}
					</p>
				</div>

				<div class="d-flex gap-2 mt-4">
					<a href="{{route('roles.edit', $role->id)}}"
						class="btn btn-warning waves-effect waves-light">
						<i class="mdi mdi-file-edit-outline"></i>
						{{__('admin.edit')}}
					</a>
					<a href="{{route('roles.index')}}"
						class="btn btn-secondary waves-effect waves-light">
						<i class="mdi mdi-arrow-left"></i> {{__('admin.back')}}
					</a>
				</div>
			</div>
		</div>
	</div>
</div>
<!-- end row -->
@endsection